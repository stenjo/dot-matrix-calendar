...
size_t parse(ics_t *ics, const char *ics_data, size_t data_size) {
    ...
    size_t new_size = ics->buffer_size + data_size;
    char *new_buffer = (char*)malloc(new_size + 1);
    if (new_buffer == NULL) {
        return ics->count; // Memory allocation failed
    }

    // Copy existing buffer to new buffer
    if (ics->buffer != NULL) {
        memcpy(new_buffer, ics->buffer, ics->buffer_size);
        free(ics->buffer);
    }

    // Copy new data to new buffer
    memcpy(new_buffer + ics->buffer_size, ics_data, data_size);
    new_buffer[new_size] = '\0';

    // Update buffer and size
    ics->buffer = new_buffer;
    ics->buffer_size = new_size;

    // Start parsing from the current position
    const char *current_position = ics->buffer;
    if (ics->next != NULL) {
        current_position = ics->next;
    }

    event_t event = getEvent(current_position, &ics->next);
...
size_t parseFile(ics_t * ics, const char *file_path) {
    ...
    if (f) {
        ...
        if (buffer) {
            ...
            parse(ics, buffer, sizeof(buffer));
