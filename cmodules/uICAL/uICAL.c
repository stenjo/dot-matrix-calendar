...
STATIC mp_obj_t RRule_time(mp_obj_t self_in) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(RRule_time_obj, RRule_time);
...
MP_DEFINE_CONST_FUN_OBJ_1(rrule_next_obj, rrule_next);
...
STATIC const mp_rom_map_elem_t RRule_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR_time), MP_ROM_PTR(&RRule_time_obj) },
    { MP_ROM_QSTR(MP_QSTR_next), MP_ROM_PTR(&rrule_next_obj) },
};
